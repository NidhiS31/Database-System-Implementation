...
bool QueryProcessor::createTable() { // CREATE TABLE
...
bool QueryProcessor::insertInto() { // INSERT INTO
  DBFile table;
...
bool QueryProcessor::dropTable() { // DROP TABLE
  // delete from catalog
...
bool QueryProcessor::exists(const char* relName) {
  ifstream fin (catalog_path);
  string line;
  while (getline(fin, line))
