...
OrderMaker::OrderMaker(const OrderMaker& order): numOfAttributes(order.numOfAttributes) {
...
OrderMaker& OrderMaker::operator= (const OrderMaker& order) {
    numOfAttributes = order.numOfAttributes;
  for (size_t i = 0; i < numOfAttributes; ++i) {
    whichAtts[i] = order.whichAtts[i];
    whichTypes[i] = order.whichTypes[i];
  }
...
OrderMaker :: OrderMaker(Schema *schema) {
	...
	for (int i = 0; i < n; i++) {
		if (atts[i].myType == Int) {
			whichAtts[numOfAttributes] = i;
			whichTypes[numOfAttributes] = Int;
			numOfAttributes++;
		}
	}
	...
	for (int i = 0; i < n; i++) {
                if (atts[i].myType == Double) {
                        whichAtts[numOfAttributes] = i;
                        whichTypes[numOfAttributes] = Double;
                        numOfAttributes++;
                }
        }
	...
	// and finally the strings
        for (int i = 0; i < n; i++) {
                if (atts[i].myType == String) {
                        whichAtts[numOfAttributes] = i;
                        whichTypes[numOfAttributes] = String;
                        numOfAttributes++;
                }
        }
}

OrderMaker::OrderMaker(int na, int* atts, Type* types): numOfAttributes(na) {
...
{
    queryorder.numOfAttributes = cnforder.numOfAttributes = 0;
  FOREACH_ZIPPED(att, type, sortOrder.whichAtts, sortOrder.whichTypes, sortOrder.numOfAttributes)
    int i = findAttrIn(att, query);
    if (i>=0) {     // found this attribute in query CNF
      UNPACK2(queryorder.whichAtts[queryorder.numOfAttributes], queryorder.whichTypes[queryorder.numOfAttributes], att, type);
      UNPACK2(cnforder.whichAtts[cnforder.numOfAttributes], cnforder.whichTypes[cnforder.numOfAttributes], i, type);
      ++queryorder.numOfAttributes, ++cnforder.numOfAttributes;
...
void OrderMaker::growFromParseTree(NameList* gAtts, Schema* inputSchema) {
  for(; gAtts; gAtts = gAtts->next, numOfAttributes++) {
    FATALIF ((whichAtts[numOfAttributes] = inputSchema->Find(gAtts->name)) == -1,
             "Grouping by non-existing attribute.");
    whichTypes[numOfAttributes] = inputSchema->FindType(gAtts->name);
  }
}
...
std::ostream& operator<<(std::ostream& os, const OrderMaker& myorder) {
  os << myorder.numOfAttributes << ' ';
  for(int i=0; i<myorder.numOfAttributes; ++i) os << myorder.whichAtts[i] << ' ';
  for(int i=0; i<myorder.numOfAttributes; ++i) os << myorder.whichTypes[i] << ' ';
  os << std::endl;
  return os;
}

std::istream& operator>>(std::istream& is, OrderMaker& myorder) {
  is >> myorder.numOfAttributes;
  for(int i=0; i<myorder.numOfAttributes; ++i) is >> myorder.whichAtts[i];
  for(int i=0; i<myorder.numOfAttributes; ++i) {
    int t; is >> t;
...
int CNF :: GetSortOrders (OrderMaker &left, OrderMaker &right) {
	...
	left.numOfAttributes = 0;
	right.numOfAttributes = 0;
	...
	for (int i = 0; i < numAnds; i++) {
		...
		if (orList[i][0].operand1 == Left) {
			left.whichAtts[left.numOfAttributes] = orList[i][0].whichAtt1;
			left.whichTypes[left.numOfAttributes] = orList[i][0].attType;
		}	

		if (orList[i][0].operand1 == Right) {
                        right.whichAtts[right.numOfAttributes] = orList[i][0].whichAtt1;
                        right.whichTypes[right.numOfAttributes] = orList[i][0].attType;
                }

		if (orList[i][0].operand2 == Left) {
                        left.whichAtts[left.numOfAttributes] = orList[i][0].whichAtt2;
                        left.whichTypes[left.numOfAttributes] = orList[i][0].attType;
                }

		if (orList[i][0].operand2 == Right) {
                        right.whichAtts[right.numOfAttributes] = orList[i][0].whichAtt2;
                        right.whichTypes[right.numOfAttributes] = orList[i][0].attType;
                }

		// note that we have found two new attributes
		left.numOfAttributes++;
		right.numOfAttributes++;
	}
	
	return left.numOfAttributes;
